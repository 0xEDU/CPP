...
/*   Updated: 2023/05/03 12:10:42 by etachott         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "PhoneBook.hpp"

static void	addContactToList(PhoneBook &phoneBook, int index) {
	std::string fn;
	std::cout << "First name: ";
	getline(std::cin, fn);
	if (fn.empty())
...
static void	returnIndex(PhoneBook &phoneBook, std::string index) {
	if (index.length() > 1 || !std::isalpha(index[0])) {
		std::cout << "Invalid index!" << std::endl;
		return ;
	}
	if (std::isdigit(index[0]) && (index[0] == char('9')
			|| index[0] == char('8'))) {
		std::cout << "Index out of range!" << std::endl;
		return ;
	}
	std::cout << "Contact:" << std::endl;
	std::cout << phoneBook.contactsList[index[0] - '0'].getFirstName()
		<< std::endl;
	std::cout << phoneBook.contactsList[index[0] - '0'].getLastName()
		<< std::endl;
	std::cout << phoneBook.contactsList[index[0] - '0'].getNickName()
		<< std::endl;
	std::cout << phoneBook.contactsList[index[0] - '0'].getPhoneNumber()
		<< std::endl;
	std::cout << phoneBook.contactsList[index[0] - '0'].getDarkestSecret()
		<< std::endl;
}
